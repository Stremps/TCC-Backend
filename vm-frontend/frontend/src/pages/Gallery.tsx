...
export function Gallery() {
  ...
  // Hook Inteligente: Busca dados e atualiza a cada 5s
  const { jobs, loading, error } = useJobsPolling();

  // Lógica de Download Real
  const handleDownload = async (id: string) => {
    try {
        // Pede a URL assinada (válida por 10min)
        const downloadUrl = await jobsService.getDownloadUrl(id);
        
        // Abre o download em nova aba (o navegador cuida de salvar)
        window.open(downloadUrl, '_blank');
    } catch (err) {
        console.error("Erro no download:", err);
        alert("Não foi possível iniciar o download. Tente novamente em instantes.");
    }
  };

  // Filtragem local (Busca por Prompt ou ID)
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex-1 px-4 max-w-7xl mx-auto w-full pb-10 pt-10">
         ...
         {/* Estado 3: Lista Vazia (Empty State) */}
         ...
         {/* Estado 4: Grid de Resultados */}
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredJobs.map(job => (
                <JobCard 
                    key={job.id} 
                    job={job} // Passando o objeto real da API
